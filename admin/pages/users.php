...
<div data-bulk-table="users">
    ...
    <div class="row align-items-center mb-4 g-3">
        ...
        <div class="col-md-6 text-md-end d-flex gap-2 justify-content-md-end align-items-center flex-wrap">
            <!-- Toolbar bulk (muncul otomatis saat ada yang dicentang) -->
            <div data-bulk-toolbar class="d-none d-flex align-items-center gap-2">
                <span class="text-muted small" data-bulk-count></span>
                <button class="btn btn-sm btn-danger rounded-pill px-3" data-bulk-delete>
                    <i class="fas fa-trash-alt me-1"></i> Hapus Terpilih
                    (<span data-bulk-count-num>0</span>)
                </button>
            </div>
        ...
        <div class="table-responsive table-responsive--no-scroll">
            <table class="table align-middle admin-compact mb-0">
                <thead>
                    <tr>
                        <th style="width:40px;">
                            <input type="checkbox" class="js-bulk-select-all" title="Pilih semua">
                        </th>
                        <th>#</th>
                        <th>Nama Peserta</th>
                        <th>Email</th>
                ...
                <tbody>
                    <?php if (empty($users)): ?>
                        <tr><td colspan="8" class="text-center text-muted py-5">Belum ada peserta yang terdaftar.</td></tr>
                    <?php else: ?>
                        <?php foreach ($users as $idx => $u): ?>
                            <tr>
                                <td>
                                    <input type="checkbox" class="js-bulk-row" value="<?php echo $u['id']; ?>">
                                </td>
                                ...
                                <td>
                                    <button class="btn btn-sm btn-soft-danger rounded-pill px-2"
                                            onclick="deleteItem('users', <?php echo $u['id']; ?>)"
                                            title="Hapus peserta">
