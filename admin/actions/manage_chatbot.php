...
if ($action === 'create' || $action === 'update') {
    ...
    try{ $ph=implode(',',array_fill(0,count($ids),'?')); $stmt=$pdo->prepare("DELETE FROM chatbot_intents WHERE id IN ($ph)"); $stmt->execute($ids); echo json_encode(['status'=>'success','message'=>$stmt->rowCount().' intent dihapus']); }catch(PDOException $e){ echo json_encode(['status'=>'error','message'=>'Gagal hapus massal']); }
    exit;
} else {
