    ...
    if (isset($_GET['ajax'])) {
        ...
        if (empty($orders)) {
            ...
            foreach ($orders as $o) {
                ...
                $isAwaiting = (($o['payment_method'] ?? '') === 'manual_transfer') && (($o['status'] ?? '') === 'pending') && (($o['payment_status'] ?? '') !== 'paid');
                if ($isAwaiting) { $stBadge = '<span class="badge badge-soft-purple"><i class="fas fa-user-check me-1"></i>Menunggu Konfirmasi</span>'; }
                elseif ($o['status'] === 'pending') { $stBadge = '<span class="badge badge-soft-warning"><i class="fas fa-clock me-1"></i>Pending</span>'; }
